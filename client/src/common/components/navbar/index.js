...
import { IoLogoGithub, IoLogoYoutube, IoInformationCircleSharp } from 'react-icons/io5';
...
export default function NavBar({token,
    ...
    return (
        <Navbar collapseOnSelect expand="lg" bg="light" variant="light" sticky="top">
            <Navbar.Brand href="/">
                {
                    page === 'feed' || 'project' ? <MdBubbleChart style={{fontSize: '40px'}}/> : null
                }
            </Navbar.Brand>
        <Navbar.Toggle aria-controls="responsive-navbar-nav" />
        <Navbar.Collapse id="responsive-navbar-nav">
            <Nav className="mr-auto" style={{fontSize: page === 'project' && '28px', fontWeight: page === 'project' && 'bold'}}>
                {
                    page === 'feed' ? `Welcome, ${username}!` : page === 'project' ? `${project.name}` : null
                }
            </Nav>
            <Nav>
                {
