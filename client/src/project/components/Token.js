...
export default function Token({
  ...
  const [suggestedToken, setSuggestedToken] = useState(
    tokenInfo1.suggested_replacement ? tokenInfo1.suggested_replacement : null
  );
  // Set current token as replacement or suggested_token if available
  const [currentToken, setCurrentToken] = useState(
    replacedToken
      ? replacedToken
      : tokenInfo1.suggested_replacement
      ? tokenInfo1.suggested_replacement
      : value
  );
  ...
  const [inputWidth, setInputWidth] = useState(
    `${(currentToken.length + 2) * 8}px`
  ); // dynamically changes input width based on text value

  // Popover Controllers
  const [showPopover, setShowPopover] = useState(false);
  const [showRemovePopover, setShowRemovePopover] = useState(false);
  const [showAddSuggestionPopover, setShowAddSuggestionPopover] =
    useState(false);

  useEffect(() => {
    // Updates token colour based on state of token information and subsequent classification
    // console.log(bgColourMap)
    const bgColourKey = Object.keys(tokenInfo1.meta_tags).filter(
      (tag) => tokenInfo1.meta_tags[tag]
    );
  ...
  useEffect(() => {
    ...
    if (
      !edited &&
      !replacedToken &&
      Object.keys(replacementDict).includes(originalToken)
    ) {
      //  && !suggestedToken &&
  ...
  const addReplacement = async (isSingle) => {
    ...
    if (isSingle && response.status === 200) {
      ...
      const response = await axios.patch(
        ...
        { original_token: originalToken, replacement: currentToken }
      );
      if (response.status === 200) {
        //console.log('replace response -> ', response.data);
        if (tokenId === localStorage.getItem("id")) {
          setReplacementDict((prevState) => ({
            ...prevState,
  ...
  const addSuggestedReplacement = async (isSingle) => {
    ...
    const response = await axios.patch("/api/token/suggest/add/single/", {
      token_id: tokenId,
      text_id: textId,
      suggested_replacement: currentToken,
    });
    if (response.status === 200) {
      // isSingle &&
  ...
  useEffect(() => {
    ...
    Object.keys(metaTagSuggestionMap) // access meta tag keys (en, rp, etc.)
      .filter(
        (metaTag) => Object.keys(metaTagSuggestionMap[metaTag]).length > 0
      ) // Check whether key has any values e.g. 'ds': {rods: true, steels: true} etc.
      .map((metaTag) => {
        ...
        if (Object.keys(metaTagSuggestionMap[metaTag]).includes(currentToken)) {
          ...
          const metaTagUpdate = {
            ...tokenInfo1.meta_tags,
            [metaTag]: Object.values(metaTagSuggestionMap[metaTag])[0],
          };
          setTokenInfo1((prevState) => ({
            ...prevState,
  ...
  const addMetaTag = async (field, value, isSingle) => {
    ...
    if (isSingle) {
      ...
      const response = await axios.patch(
        ...
        { original_token: originalToken, field: field, value: value }
      );
      if (response.status === 200) {
        const metaTagUpdate = { ...tokenInfo1.meta_tags, [field]: value };
        setTokenInfo1((prevState) => ({
        ...
        setToastInfo({
          ...
          content: {
            ...
            metaTag: field,
            metaTagValue: value,
            count: response.data.matches,
          },
        });
      }

      // Add meta tag to suggestion map
      const subMetaTagMapUpdated = {
        ...metaTagSuggestionMap[field],
        [currentToken]: value,
      };
      setMetaTagSuggestionMap((prevState) => ({
        ...prevState,
        [field]: subMetaTagMapUpdated,
      }));
      setMetaTagUpdated(true);
    }
  };
        ...
        <>
          ...
          <div
            style={{
              display: "flex",
              flexDirection: "row",
              justifyContent: !suggestedToken ? "space-between" : null,
              width: inputWidth,
            }}
          >
            <TokenUnderline {...TokenUnderlineProps} />
          </div>
