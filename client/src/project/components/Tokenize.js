  ...
  const applyTokens = async () => {
    if (valid) {
      ...
      const textIndexes = new Set(
        textIntermediate.tokens.map((token) => token.index)
      );
      const tokenIndexesTK = new Set(
        [...textIndexes].filter((x) => !tokenIndexes.has(x))
      );
      const response = await axios.patch("/api/text/tokenize", {
        text_id: textIntermediate._id,
        project_id: project._id,
        index_groups_tc: Array.from(tokenIndexGroups),
        indexes_tk: Array.from(tokenIndexesTK),
      });
      if (response.status === 200) {
          ...
          {textIntermediate.tokens.map((token) => {
            return (
              <div
                ...
                style={{
                  backgroundColor: tokenIndexes.has(token.index)
                    ? "#BFE3BF"
                    : "#fdfd96",
                  width:
                    (token.value.length + 2) * 10 < MIN_TOKEN_WIDTH
                      ? `${MIN_TOKEN_WIDTH}px`
