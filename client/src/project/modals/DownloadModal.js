    ...
    const fetchProjectMaps = async () => {
      if (!mapsLoaded) {
        ...
        if (response.status === 200) {
          ...
          setMaps(
            response.data.map_keys.filter((key) => !DEFAULT_MAPS.includes(key))
          );
          setMapsLoaded(true);
        }
      }
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title>
          <MdLibraryBooks style={{ marginRight: "0.5em" }} />
          Download Results
        </Modal.Title>
      </Modal.Header>
      ...
      <Modal.Body>
        <p>
          Normalisation and gazetteers for project{" "}
                  ...
                  <tr key={index}>
                    ...
                    <td>
                      <MdFileDownload
                        ...
                        onClick={() => downloadMaps(project, mapName)}
                      />
                    </td>
                  </tr>
                ))
      ...
      <Modal.Footer>
        <Button variant="secondary" onClick={() => setShowDownload(false)}>
          Close
        </Button>
      </Modal.Footer>
    </Modal>
